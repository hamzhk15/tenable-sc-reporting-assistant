...
# -*- coding: utf-8 -*-
"""
upload_rest.py -- import a generated dashboard/report XML into Tenable
Security Center using ONLY the Python standard library (no pyTenable, no pip).

Talks directly to the SC REST API with API-key auth:

  1. POST /rest/file/upload            (multipart)  -> {"response":{"filename": token}}
  2. POST /rest/dashboard/import       {"name","filename"}     (dashboards)
...
Examples:
    ...
    python3 upload_rest.py --dashboard "…- Dashboard.xml" --insecure
    python3 upload_rest.py --report "…- Report.xml" --name "Q3 VM Plan"
